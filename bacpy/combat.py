...
def design_mat(mod, numerical_covariates, batch_levels, printFuct):
    ...
    printFuct("    found %i batches" % design.shape[1])
...
def combat(data, batch, printFuct, model=None, numerical_covariates=None):
    ...
    design = design_mat(model, numerical_covariates, batch_levels, printFuct)

    #sys.stderr.write("Standardizing Data across genes.\n")
    B_hat = np.dot(np.dot(la.inv(np.dot(design.T, design)), design.T), data.transpose())
