...
def optimize_model_platereader(
    ...
    This function performs a random search across various model types (Random Forest, 
    Extra Trees, Neural Nets, LightGBM, CatBoost). It uses Stratified K-Fold 
    cross-validation to ensure each fold maintains the same class distribution 
    as the original dataset, providing a robust estimate of model performance 
    on spectral fingerprints.

    Args:
        rf_dat: The processed wide-format Polars DataFrame containing features (wv_*) 
            and target labels.
        on: The taxonomic rank or label column to optimize for (e.g., "strainID").
        by: The metric used to sort the final results (e.g., "f1", "accuracy", "mcc").
        cv_folds: The number of folds for stratified cross-validation.
    ...
    Returns:
        A Polars DataFrame containing performance metrics for every fold and 
        parameter combination, sorted by the specified metric.
    """
...
def get_optimized_model(
    ...
    This function parses the stringified class paths and hyperparameter dictionaries 
    from the first row of an optimization result. It dynamically imports the 
    required modules and reconstructs the model instance, ready for final training 
    on a full dataset.

    Args:
        optimization_result: A Polars DataFrame generated by `optimize_model_platereader`, 
            where the first row contains the best-performing configuration.
        n_jobs: The number of threads to assign to the new model instance.
        filename: Optional path to save the instantiated model using `save_model`.

    Returns:
        An instance of the best classifier (e.g., classifier_randomForest) 
        initialized with optimized hyperparameters.

    Raises:
        ImportError: If the class path stored in the result cannot be imported.
        AttributeError: If the specified class name is not found in the module.
    """
